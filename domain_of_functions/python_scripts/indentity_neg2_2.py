...
"""We will construct f(x) = x on
[-2, 2]. Then, we will output the domain and range
...
for i, j in enumerate(xs):
    with open('identity_neg2_2.table', 'w') as file:
        for u in j:
            if f(u) >= -2 and f(u) <= 2:
                file.write('{:f} {:f}\n'.format(u, f(u)))
...
for s, t in enumerate(dom_xs):
    with open('domain_identity_neg2_2.table', 'w') as file:
        for v in t:
            if v >= -2 and v <= 2:
                file.write('{:f} {:f}\n'.format(v, dom_f(v)))